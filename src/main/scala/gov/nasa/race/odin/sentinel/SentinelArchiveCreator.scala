...
/** offline tool to merge Sentinel query responses into a TaggedTextArchive containing JSON payload */
...
/**
 * utility class to merge a set of single device Sentinel server query responses into one time-sorted TaggedTextArchive
 * Used offline
 */
